...
- [ ] **DATA-01**: Three source CSVs (vehicle availability, light-duty utilization, ferry ticket counts) load into DuckDB Bronze tables with explicit types and fail-fast row-count assertions (4,614 / 2,086 / 272,529)
...
- [ ] **DATA-03**: The 209 null `AVAILABILITY_YTD` values are preserved as genuine NULL (never coerced to 0) and excluded from rate calcs (denominator 4,405), flagged as a DQ gap with no imputation
