...
**Plans**: 3 plans
  - [x] 06-01-PLAN.md — `[build-system]` (hatchling) + `uv sync` (importable package) + `.gitattributes` (CSV eol=lf, Parquet binary) + `.gitignore` build artifacts (SHIP-01)
  - [x] 06-02-PLAN.md — `pipeline.py` full-chain orchestrator (one-command run) + `tests/test_pipeline.py` smoke test + clean no-op diff verification (SHIP-01)
  - [x] 06-03-PLAN.md — README (citations, assumptions, pull date, dated test gate, headline numbers) + REQUIREMENTS.md traceability reconcile + self-consistency cross-check + human review (SHIP-01)
